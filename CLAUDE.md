...
runtime in C# / .NET 10, built in 8 phases. **The repo is currently at Phase 0
(scaffold only)** — no container functionality (`run`, `pull`, namespaces,
cgroups, registry client) exists yet. Later phases add: command parsing (1),
P/Invoke (2), chroot/pivot_root (3), namespaces (4), cgroup v2 (5), rootless
mode (5), image pull + registry client (7–8).
...
dotnet test --filter FullyQualifiedName~ScaffoldTests   # run a single test/class
...
- `src/CCRun/` — the CLI console app (`net10.0`). `Program.cs` is a top-level-statement
  entrypoint returning an int exit code.
- `tests/CCRun.Tests/` — xUnit tests, references `src/CCRun`.
